...
A reproducible FastAPI application for deterministic claim screening, evidence retrieval, and coarse stance analysis, built from a Queen Mary University of London dissertation project on conflict-news claim detection.

> This project is a cleaned, public dissertation demo. It does **not** claim to be a production fact-checker, and it does **not** present synthetic evaluation results as real-world model accuracy.

## Live Demo

Live deployment is prepared for Vercel with [`vercel.json`](vercel.json) and [`api/index.py`](api/index.py). A public URL has not been verified yet for this repository.
...
- Includes reproducible evaluation, latency benchmarks, Vercel configuration, and GitHub Actions CI.
...
|---|---:|---:|
| Claim scoring | `0.0436` | `0.0492` |
| Evidence ranking | `2.1576` | `2.3275` |
| Full analysis | `2.2146` | `2.3142` |
...
ruff check .
ruff format --check .
python -m compileall api claim_detection evaluation benchmarks tests
pytest
```
...
- Vercel entrypoint import validation
...
Vercel is the target deployment platform for the public dissertation demo. The app uses Vercel's Python runtime to serve the FastAPI application from [`api/index.py`](api/index.py).
