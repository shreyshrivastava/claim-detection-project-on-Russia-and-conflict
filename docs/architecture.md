...
flowchart TB
    ...
    Notebook["Historical BERT/SVM notebooks"] -. "not deployed" .-> Audit["Generalization audit guidance"]
```
...
- `claim_detection/api.py`: FastAPI app, validation, health check, and demo UI.
...
- `evaluation/generalization_audit.py`: split leakage and overfitting audit helper.
