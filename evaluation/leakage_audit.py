"""Audit data splits for leakage, duplicate examples, and overfit signals."""
...
class PredictionRecord:
    ...
    @property
    def leakage_key(self) -> str | None:
        if self.text_key:
            return self.text_key
        if self.claim and self.evidence:
            normalized = " ".join(f"{self.claim} {self.evidence}".lower().split())
            return hashlib.sha256(normalized.encode("utf-8")).hexdigest()
        if self.claim:
            normalized = " ".join(self.claim.lower().split())
            return hashlib.sha256(normalized.encode("utf-8")).hexdigest()
        return None
...
def load_prediction_csv(path: Path) -> list[PredictionRecord]:
    ...
    Optional leakage columns: `text_key`, `claim_id`, `claim`, `evidence`.
    """
    ...
    with path.open(newline="") as handle:
        ...
        return [
            PredictionRecord(
                ...
                claim=(row.get("claim") or "").strip() or None,
...
def audit_splits(splits: dict[str, list[PredictionRecord]]) -> dict[str, Any]:
    """Compute metrics and leakage warnings for provided prediction splits."""
    ...
    for split_name, records in splits.items():
        for record in records:
            key = record.leakage_key
            if key:
                key_to_splits[key].add(split_name)
    ...
    if duplicate_keys:
        warnings.append(
            "Duplicate claim/evidence keys appear across splits; this can inflate validation/test metrics."
        )
    ...
    return {
        "metrics": metrics,
        "duplicate_claim_evidence_keys": duplicate_keys[:100],
        "duplicate_claim_evidence_key_count": len(duplicate_keys),
        "warnings": warnings,
    }
...
def write_audit(results: dict[str, Any], output_dir: Path) -> None:
    ...
    (output_dir / "leakage-audit.json").write_text(json.dumps(results, indent=2) + "\n")
    lines = [
        "# Leakage Audit",
        "",
        "Use this report when real train/validation/test predictions are available. "
        "It catches duplicate claim/evidence keys, suspicious perfect metrics, "
        "small split sizes, and large train/validation/test gaps.",
        "",
        "## Metrics",
        "",
    ]
    ...
    if not results["warnings"]:
        lines.append("- No leakage or overfitting warning thresholds were triggered.")
    lines.append("")
    (output_dir / "leakage-audit.md").write_text("\n".join(lines))
